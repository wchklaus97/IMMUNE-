...
const V85_IMMUTABLE_FILES = new Map([
  ...
  ["tools/validate_v8_5_export.mjs", "48aee124a1fcd90516ec80ca088bbc37b3bd71991e29af6751a5a4e3808db85d"],
  ["tools/validate_v8_5_export.test.mjs", "9d94e0febb33047589549b7c97bc4365aedac55e546bc6196b83a219dc038b39"],
  ["tools/godot/v8_5_candidate_probe.gd", "57a2e4e8b10c23cec5eaf84bd59b222f8019887d0667cafad042b4c8aa59ff7b"],
]);
...
export async function validateV86ExportContract({ root = ROOT, pck = "" } = {}) {
  const errors = [];
  const [projectSource, presetsSource, packageSource, ciSource, profileSource, authoredBodySource, probeSource] = await Promise.all([
  ...
  for (const name of SHIPPING_NAMES) {
    ...
    requireCondition(errors, presetExcludesResource(preset, BODY_RESOURCE), `${name}: active V8.6 R7.2 body must remain excluded`);
  ...
  const probeChecks = [
    ...
    [probeSource.includes("V8_6_CANDIDATE_PROBE_OK"), "probe success marker is missing"],
  ];
