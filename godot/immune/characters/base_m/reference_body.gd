...
const VARIANTS := {
	...
	"fizzy": {
		...
		&"light_wrap": 0.30,
		&"sss_amount": 0.78,
		&"transmit_strength": 1.25,
		&"thin_glow": 0.50,
		&"rim_energy": 0.24,
		&"interior_budget": 0.18,
		&"rim_budget": 0.11,
		&"body_budget": 0.94,
		&"body_absorb": 0.64,
		&"extinction_density": 2.2,
		&"extinction_spread": 1.2,
		&"extinction_shape": 1.9,
		&"dimple_scale": 145.0,
		&"dimple_depth": 0.0,
		&"dimple_crease": 0.18,
		&"dimple_round": 0.95,
		...
		&"bubble_jitter": 0.17,
		&"bubble_softness": 0.080,
		&"bubble_depth": 0.004,
		&"bubble_thinness": 0.42,
		&"bubble_shell_shadow": 0.0,
		&"bubble_emission": 0.018,
		&"bubble_shell_emission": 0.32,
		&"bubble_seed": 59.0,
		&"microbubble_enabled": true,
		&"microbubble_scale": 44.0,
		&"microbubble_density": 0.96,
		&"microbubble_radius_min": 0.10,
		&"microbubble_radius_max": 0.30,
		&"microbubble_jitter": 0.17,
		&"microbubble_softness": 0.090,
		&"microbubble_depth": 0.004,
		&"microbubble_thinness": 0.22,
		&"microbubble_shell_shadow": 0.0,
		&"microbubble_emission": 0.024,
		&"microbubble_shell_emission": 0.27,
		&"microbubble_seed": 89.0,
		&"inclusion_enabled": true,
		&"inclusion_scale": 180.0,
		&"inclusion_threshold": 0.64,
		&"inclusion_softness": 0.050,
		&"inclusion_emission": 0.17,
...
func _make_shell(variant: String) -> ShaderMaterial:
	...
	shell.set_shader_parameter(&"edge_alpha", 0.68 if variant == "clear" else (0.58 if variant == "fizzy" else 0.44))
	shell.set_shader_parameter(&"edge_power", 3.0)
	shell.set_shader_parameter(&"shell_roughness", 0.032 if variant != "gummy" else 0.055)
	shell.set_shader_parameter(&"rim_emission", 0.60 if variant != "gummy" else 0.38)
